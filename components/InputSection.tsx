...
const InputSection: React.FC<InputSectionProps> = ({
  ...
  React.useEffect(() => {
    if (isLoading) {
      ...
      setProgress(100);
      const timer = setTimeout(() => setProgress(0), 500); // Clear after delay
          ...
          <div>
            ...
            {!mediaFile ? (
              <div
                ...
                className="group border border-dashed border-slate-700 hover:border-indigo-500/50 bg-slate-950/50 rounded-lg p-8 text-center cursor-pointer transition-all"
              >
        ...
        <div className="space-y-4">
              ...
              {modeOptions.map((option) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center justify-center gap-1.5 py-2.5 rounded text-sm font-medium transition-all ${isSelected
            ...
            <div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`p-3 rounded-lg border transition-all ${searchEnabled
                    ? 'bg-indigo-950/30 border-indigo-500/50 text-indigo-300'
                    : 'bg-slate-950 border-slate-800 text-slate-500'
                    }`}
                  title={searchEnabled ? '検索ON' : '検索OFF'}
                >
          ...
          <div>
            ...
            <button
              ...
              className={`w-full flex items-center justify-between p-4 rounded-lg border transition-all ${enableVideoAnalysis
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-base font-medium">{enableVideoAnalysis ? 'ON (映像・音声を解析)' : 'OFF (メタデータのみ)'}</span>
              </div>
        ...
        {isLoading ? (
          <>
            <div className="flex items-center gap-3">
              ...
              <span className="text-lg">動画・情報を分析中... {progress}%</span>
            </div>
            {/* Progress Bar */}
            <div className="w-full max-w-sm mt-3">
              ...
              <p className="text-xs text-slate-400 mt-1.5 font-normal">AIが解析しています。しばらくお待ちください...</p>
            </div>
          </>
        ) : (
