...
const SongCard: React.FC<SongCardProps> = ({ selection, label, isAlternative = false, onMinimaxGenerate }) => {
    ...
    // Helper: convert MVTimeline object to readable display string
    ...
    // Helper: get timeline as display string regardless of type
    ...
    const handleDownloadJson = () => {
        ...
        if (typeof selection.timeline === 'object') {
            // Structured MVTimeline object from Gemini API — use directly
            jsonData = {
                title: title,
                style: style,
            ...
            // Fallback: legacy text-based timeline (should not happen with current API)
            jsonData = {
                title: title,
                style: style,
                lyrics: formatContent(content),
...
const ResultSection: React.FC<ResultSectionProps> = ({ data, onTitleSelect, isGeneratingPhase2, onMinimaxGenerate, phase2Progress = 0, phase2Message = '生成中...' }) => {
    return (
                ...
                <div className="text-center py-12 bg-slate-900/50 border border-dashed border-slate-700 rounded-2xl">
                    {isGeneratingPhase2 ? (
                        <div className="max-w-md mx-auto space-y-4">
                            ...
                            <p className="text-slate-200 text-lg font-medium">{phase2Message} {phase2Progress}%</p>
                            <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                                <div
                                    className="h-full bg-gradient-to-r from-pink-500 via-fuchsia-500 to-violet-500 transition-all duration-500 ease-out"
                                    style={{ width: `${phase2Progress}%` }}
                                />
                            </div>
                            <p className="text-slate-500 text-sm">歌詞、構成、MV 用タイムラインを順番にまとめています。</p>
                        </div>
                    ) : (
                        <>
