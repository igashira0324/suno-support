...
import { fetchUrlMetadata, buildFirecrawlQuery, searchWithFirecrawl } from "./searchUtils";
export { structureLyrics } from "./lyricsUtils";
...
export const generateSunoPrompt = async (
    ...
    options: { searchEngine: SearchEngine; modelName: string; enableVideoAnalysis?: boolean; lyricsLanguage?: string } = { searchEngine: 'google-grounding', modelName: 'gemini-3.5-flash', enableVideoAnalysis: false, lyricsLanguage: 'Japanese' },
    theme: string = ""
    ...
    try {
        ...
        // Use the model selected by user, default to gemini-3.5-flash
        ...
        let externalSources: GroundingSource[] = [];
        ...
        if (options.searchEngine === 'firecrawl') {
            const firecrawlQuery = buildFirecrawlQuery({ text, youtubeUrl, theme, mode, urlMetadata });
            const firecrawlSearch = await searchWithFirecrawl(firecrawlQuery);
            externalSources = firecrawlSearch.sources;

            if (firecrawlSearch.context) {
                prompt += `\n=== FIRECRAWL WEB RESEARCH ===\nQuery: ${firecrawlQuery}\n${firecrawlSearch.context}\n`;
                prompt += `\nINSTRUCTION: Prioritize the FireCrawl research above when inferring genre traits, artist context, instrumentation, and lyrical themes. Cite the specific retrieved context in your internal reasoning, but keep the final output in the required JSON schema.\n`;
            }
        }
        ...
        while (retryCount <= maxRetries) {
            try {
                ...
                if (externalSources.length > 0) {
                    jsonResponse.sources = externalSources;
                }
...
export const generateFromSelectedTitle = async (
    ...
    options: { modelName: string; lyricsLanguage?: string } = { modelName: 'gemini-3.5-flash', lyricsLanguage: 'Japanese' }
