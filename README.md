...
以下のコンポーネントをまとめて起動します。

- **Frontend**: http://localhost:3300
- **FastAPI Backend**: http://127.0.0.1:8100
- **ACE-Step Engine**: http://127.0.0.1:8101
...
または `run_local.sh` を実行しているターミナルで `Ctrl+C` を押してください。
...
`run_local.sh` / `run_backend.sh` / `run_frontend.sh` は `proxy_profiles/current.env` を自動で読み込みます。
...
### 3. 補足
- フロントエンドの実ポートは `3300` です。
- ACE-Step は初回ロードに 1〜2 分程度かかることがあります。
- FireCrawl は `../firecrawl` が存在する場合に自動起動します。
...
- **任意**: `GOOGLE_CUSTOM_SEARCH_CX`
- **任意**: `TAVILY_API_KEY`
- **任意**: `FIRECRAWL_API_URL` (`http://localhost:3002` 推奨)
- **任意**: `FIRECRAWL_API_KEY`
